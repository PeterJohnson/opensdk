...
strip() {
    ...
    for lib in ${TREEIN_DIR}/${TOOLCHAIN_NAME}/*/sysroot/lib64/* ${TREEIN_DIR}/${TOOLCHAIN_NAME}/*/sysroot/*/*/* ${TREEIN_DIR}/${TOOLCHAIN_NAME}/*/sysroot/usr/lib/*/*/*/*; do
        if file "${lib}" | grep -qiF -e "elf "; then
            "${STRIP_CMD}" --strip-unneeded "${lib}" || die "Could not strip ${lib}"
        fi
    done
